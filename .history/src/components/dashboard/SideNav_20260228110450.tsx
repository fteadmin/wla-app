...
export default function SideNav({
  ...
  return (
    <>
      ...
      >
        {/* --- FLOATING TAB BUTTON --- */}
        {/* Moves with the sidebar, stays on the right edge */}
        <button
          onClick={onClose}
          className={`
            lg:hidden absolute top-[10px] -right-[50px] w-[40px] h-[40px] 
        ...
        {/* Scrollable Nav Area */}
        <div className="flex-1 overflow-y-auto relative z-10 px-2 pt-4">
          <div className="px-2 pb-1.5 text-[9px] font-bold tracking-widest uppercase text-[#a0a0b4]/60">Navigation</div>
        ...
        <div className="relative z-10 p-2 border-t border-[#D9BA84]/10">
          <div className="mb-2 flex items-center gap-2.5 px-3 py-2 rounded-xl bg-[#D9BA84]/5 border border-[#D9BA84]/10">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold text-[#D9BA84] bg-[#D9BA84]/10">{memberInitials}</div>
            <div className="flex-1 min-w-0">
              <div className="text-[12px] font-semibold text-white truncate">{memberName}</div>
              <div className="text-[9px] text-[#D9BA84]">{memberTier}</div>
            </div>
          </div>
          <button className="flex items-center gap-2.5 px-3 py-2 text-[13px] text-[#a0a0b4] hover:text-red-400 transition-colors w-full text-left">
