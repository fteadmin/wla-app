...
async function fetchTokenHistory(userId: string) {
  return [
    { id: 1, type: "earn", label: "Contest Win", amount: 500, price: "$0", date: "2026-02-20", icon: Trophy },
    { id: 2, type: "spend", label: "Redeemed T-Shirt", amount: -1000, price: "$0", date: "2026-02-22", icon: ShoppingBag }
  ];
}
type UserProfileRow = Database["public"]["Tables"]["user_profiles"]["Row"];

function StripeModal({ pack, onClose, onSuccess }) {
  ...
  const handlePay = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/token-payment-intent", {
        ...
        body: JSON.stringify({ tokens: pack.tokens, price: pack.price }),
      });
      ...
      if (paymentIntent?.status === "succeeded") {
        setDone(true);
        setTimeout(() => { onSuccess(pack.tokens, pack.price); onClose(); }, 2200);
      }
    } catch (err) {
...
function TokenBasic() {
  ...
  const handleSuccess = async (tokens, price) => {
    setLoading(true);
    const { data: auth } = await supabase.auth.getUser();
    if (auth.user) {
      const { data: profile } = await supabase
          ...
          {TOKEN_PACKS.map(pack => {
            ...
            return (
              <div key={pack.id} className={`relative bg-[#161616] border rounded-2xl p-6 flex flex-col cursor-pointer transition hover:border-[#D9BA84]/60 ${pack.highlight ? "border-[#D9BA84]/60 shadow-lg" : "border-[#D9BA84]/20"}`} onClick={() => setPack(pack)}>
                ...
                <button type="button" className={`w-full py-2 rounded-lg font-bold flex items-center justify-center gap-2 transition ${pack.highlight ? "bg-gradient-to-r from-[#D9BA84] to-[#c8b450] text-black shadow" : "bg-[#D9BA84]/10 text-[#D9BA84] border border-[#D9BA84]/30"}`} onClick={() => setPack(pack)}> <ShoppingCart size={16} /> Buy Now </button>
              </div>
            );
          })}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-[#161616] border border-[#D9BA84]/20 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button className="text-xs text-[#D9BA84] font-semibold flex items-center gap-1">See all <ChevronRight size={12} /></button>
            </div>
            {history.slice(0, 6).map(h => {
              const Ico = h.icon;
              return (
                <div key={h.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-[#D9BA84]/5 border border-transparent hover:border-[#D9BA84]/10 mb-1 transition">
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${h.type === "purchase" ? "bg-[#D9BA84]/20 text-[#D9BA84]" : h.type === "earn" ? "bg-[#c8b450]/20 text-[#c8b450]" : "bg-[#a0a0b4]/10 text-[#a0a0b4]"}`}><Ico size={15} /></div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm truncate font-semibold text-white">{h.label}</div>
                    <div className="text-xs text-[#a0a0b4] mt-0.5">{h.date}</div>
                  </div>
                  <div className="text-right">
                    <div className={`text-sm font-mono font-bold ${h.amount > 0 ? "text-[#D9BA84]" : "text-[#a0a0b4]"}`}>{h.amount > 0 ? "+" : ""}{h.amount.toLocaleString()}</div>
                    {h.price && <div className="text-xs text-[#a0a0b4]">{h.price}</div>}
                  </div>
                </div>
              );
            })}
