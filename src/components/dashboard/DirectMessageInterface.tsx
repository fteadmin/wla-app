...
import { useDirectMessages, getDisplayName, getInitials } from "@/hooks/useDirectMessages";
...
export default function DirectMessageInterface({ 
  ...
  const { messages, loading, sending, error, sendMessage } = useDirectMessages(userId, partnerId);
    ...
    <div className="flex flex-col h-[calc(100vh-60px)] max-h-[calc(100vh-60px)]">
      ...
      <div className="flex items-center gap-3 px-6 py-4 border-b border-[#D9BA84]/10 flex-shrink-0">
        {onBack && (
          <button
            onClick={onBack}
            className="w-8 h-8 rounded-lg bg-[#D9BA84]/10 border border-[#D9BA84]/20 text-[#D9BA84] flex items-center justify-center hover:bg-[#D9BA84]/20 transition-colors"
          >
            <ArrowLeft size={16} />
          </button>
        )}
        <div className="w-9 h-9 rounded-[10px] bg-[#D9BA84]/10 border border-[#D9BA84]/20 flex items-center justify-center text-[#D9BA84] text-[12px] font-bold">
          {partnerName.slice(0, 2).toUpperCase()}
        </div>
        <div>
          <h1 className="text-[15px] font-bold text-white font-sora">{partnerName}</h1>
          <p className="text-[11px] text-[#a0a0b4]">Direct message</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-1 scrollbar-thin scrollbar-thumb-[#D9BA84]/10 scrollbar-track-transparent">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
            ...
            <div>
              <p className="text-[13px] text-white font-semibold mb-1">Start the conversation</p>
              <p className="text-[12px] text-[#a0a0b4]">Send a message to {partnerName}</p>
            </div>
          </div>
        ) : (
          messages.map((msg, i) => {
            ...
            const displayName = isOwn ? "You" : getDisplayName(msg.sender);
            const initials = getInitials(msg.sender);

            return (
              <div 
                key={msg.id} 
                className={`flex gap-2.5 ${isOwn ? "flex-row-reverse" : ""} ${showHeader ? "mt-3" : "mt-0.5"}`}
              >
